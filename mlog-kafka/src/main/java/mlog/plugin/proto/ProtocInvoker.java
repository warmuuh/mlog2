...
//		}
    ...
    List<String> protocArgs = new LinkedList<>();
    protocArgs.add(inputFile);
    protocArgs.addAll(includePathArgs(inputFile));
    protocArgs.add("--include_std_types");
    protocArgs.add("--descriptor_set_out=" + descriptorPath.toAbsolutePath().toString());
  ...
  private List<String> includePathArgs(String inputFilePath) {
...
//		resultBuilder.add("-I" + wellKnownTypesInclude.toString());
    ...
    return List.of("-I" + new File(inputFilePath).getParent());
  }

  private void invokeBinary(List<String> protocArgs) throws ProtocInvocationException {
    int status;
